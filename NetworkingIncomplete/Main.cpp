...
int main(int argc, char** argv)
{
    if (enet_initialize() != 0)
    {
        ...
        cout << "An erro occurred while initializing ENet" << endl;
        return EXIT_FAILURE;
    }
    ...
    if (UserInput == 2)
    {
        if (!CreateClient())
        {
            fprintf(stderr, "An error occured creating Enet CLIENT host");
            exit(EXIT_FAILURE);
        }
        ...
        if (peer == NULL)
        {
            fprintf(stderr, "0 availible peers for initiating connection.");
            exit(EXIT_FAILURE);
        }

        if (enet_host_service(client, &event, 5000) > 0 && event.type == ENET_EVENT_TYPE_CONNECT)
        {
            cout << "Connection ending in 1234 succeeded." << endl;
        }

        else
        {
            enet_peer_reset(peer);
            cout << "Connection ending in 1234 failed." << endl;
        }
    }
